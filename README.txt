Name: Ryan Stoparczyk
Update Date: 11-03-2024

How to build:
- Navigate to proper directory in terminal
- execute: 'npm i' to install required modules.
- execute: 'npm run start'

How to access:
- Keep a terminal open, running the server.
- Naviate to 'http://localhost:3000' in your browser of choice.

Purpose: To create a CRUD program, working with node.js on the backend.
 - Languages used: JavaScript, CSS, HTML (view engine: pug)
 - Modules used: axiom, express, pug, mongoose
 - dev modules used: nodemon, prettier 

Thank you!